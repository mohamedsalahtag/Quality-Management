# AGENTS.md — Sharbatly QMS

Behavioral guide for OpenAI Codex, Cursor, GitHub Copilot, and any other AI coding agent that reads this file. Mirrors `CLAUDE.md` so all agents share the same contract.

## READ FIRST

Before doing anything in this repository, read these two files in order:

1. **`PROJECT_STATE.md`** (this folder) — live state of the QMS web app: where it runs, what is built, the decisions log, the hard rules. The single best file to bring a fresh AI session up to speed.
2. **`QMS_Extended_Assessment_and_Execution_Plan.md`** — original 16-week plan + normalized SQL schema. Source of truth for table names, status codes, role logic, build-phase order.
...
Persistent memory: Claude Code maintains its own auto-memory at `~/.claude/projects/<dir-slug>/memory/MEMORY.md`. Other agents (you) do not see that index. Treat `PROJECT_STATE.md` as the cross-agent source of truth and update it after meaningful changes.
