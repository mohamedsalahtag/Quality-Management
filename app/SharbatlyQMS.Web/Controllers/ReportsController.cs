...
public class ReportsController : Controller
{
    ...
    private static void SetMeasureCell(IXLCell cell, decimal? v, string? format)
    {
        ...
        cell.Style.NumberFormat.Format = format switch
        {
            ...
            "dec1"    => "#,##0.0",
            "dec2"    => "#,##0.00",
            // Measures like DefectPercentage/DefectRate are already 0–100 scaled,
            // so Excel's "0.00%" (which multiplies by 100) would render 12.5 as
            // "1250.00%". Use a literal "%" suffix that does NOT rescale.
            "percent" => "#,##0.00\"%\"",
